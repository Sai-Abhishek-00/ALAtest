...
if __name__ == '__main__':
    ...
    rate_list = read_operator_rate_file(filename) # rate list has the information from csv file, it contains all the [extention, rate, operator]
    extention_codes_list,max_extention_length = get_max_extention_length(rate_list) #extention_code_list has only the allowed extentions by operator. This is to minimize operations on list of lists
    input_number = get_input() # store the user defined phone number for which the rate needs to be checked
    dialing_operator = extract_operator_extention(input_number) #Store only the operator of the user entered phone number
